...
BoardVO vo = (BoardVO) Execute.run(new BoardViewServiceImpl(), new Object[] {no,inc});
%>
<!DOCTYPE html>
<html>
<head>
			...
			<table>
				<tbody>
					<tr>
						...
						<td><input name="no" id="no" readonly value=<%=vo.getNo()%>></td>
					</tr>
					<tr>
						<th>제목</th>
						<!-- input 태그의 값 세팅 : value 속성 사용 -->
						<td><input name="title" id="title" value=<%=vo.getTitle()%>></td>
					</tr>
					<tr>
						<th>내용</th>
						<!-- textarea 태그의 값 세팅 : 태그 사이 사용 -->
						<td>
							<textarea name="content" id="content" rows="7"><%=vo.getContent()%></textarea>
						</td>
					</tr>
					<tr>
						<th>작성자</th>
						<td><input name="writer" id="writer" value=<%=vo.getWriter()%>></td>
					</tr>
					<tr>
						<th>비밀번호</th>
